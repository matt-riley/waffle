...
func (s *Store) ListUpdatedAfter(ctx context.Context, updatedAt, id string, limit int) ([]Session, error) {
	...
	rows, err := s.db.QueryContext(ctx, `
		SELECT id, title, summary, model_alias, model_alias_version, created_at, updated_at, summary_watermark, reflected_at, forked_from, forked_at_seq
		FROM sessions
	...
	for rows.Next() {
		var sess Session
		var created, updated, reflected string
		if err := rows.Scan(&sess.ID, &sess.Title, &sess.Summary, &sess.ModelAlias, &sess.ModelAliasVersion, &created, &updated, &sess.SummaryWatermark, &reflected, &sess.ForkedFrom, &sess.ForkedAtSeq); err != nil {
			return nil, err
		}
