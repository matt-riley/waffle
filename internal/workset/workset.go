...
// Replace drops id and adds a new entry of the same kind.
func (s *Store) Replace(ctx context.Context, sessionID, entryID, body, source string) (*Entry, error) {
	...
	if err := s.Drop(ctx, sessionID, entryID); err != nil {
		return nil, err
	}
	return s.Add(ctx, sessionID, old.Kind, body, source, old.Pinned)
}
