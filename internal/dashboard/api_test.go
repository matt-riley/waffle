...
func TestEventsUsesAfterQueryForInitialNativeEventSourceCursor(t *testing.T) {
	hub := NewEventHub(2)
	hub.Publish(Event{Type: "one"})
	hub.Publish(Event{Type: "two"})
	current := hub.Publish(Event{Type: "three"})
	...
	request, _ := http.NewRequestWithContext(
		ctx,
		http.MethodGet,
		"http://127.0.0.1:8422/api/v1/desk/events?after="+stringCursor(current.Cursor),
		nil,
	)
	done := make(chan struct{})
	go func() {
		defer close(done)
		newEventsHandler(config).ServeHTTP(writer, request)
	}()
	ticks <- time.Now()
	writer.waitForFlush(t)
	cancel()
	...
	if got, want := writer.String(), ": heartbeat\n\n"; got != want {
		t.Fatalf("fresh cursor SSE = %q, want %q", got, want)
	}
}

func TestEventsLastEventIDTakesPrecedenceOverAfterQuery(t *testing.T) {
	hub := NewEventHub(4)
	first := hub.Publish(Event{Type: "one"})
	...
	request, _ := http.NewRequestWithContext(
		ctx,
		http.MethodGet,
		"http://127.0.0.1:8422/api/v1/desk/events?after=not-a-cursor",
		nil,
	)
	request.Header.Set("Last-Event-ID", stringCursor(first.Cursor))
	done := make(chan struct{})
	go func() {
		defer close(done)
		newEventsHandler(config).ServeHTTP(writer, request)
	...
	want := "id: 2\nevent: two\ndata: {\"cursor\":2,\"type\":\"two\"}\n\n"
	if got := writer.String(); got != want {
		t.Fatalf("header precedence SSE = %q, want %q (second cursor %d)", got, want, second.Cursor)
	}
}

func TestEventsRejectsInvalidAfterQuery(t *testing.T) {
	for _, rawQuery := range []string{
		...
		t.Run(rawQuery, func(t *testing.T) {
			...
			request, _ := http.NewRequestWithContext(
				ctx,
				http.MethodGet,
				"http://127.0.0.1:8422/api/v1/desk/events?"+rawQuery,
				nil,
			)
			newEventsHandler(testAPIConfig(t)).ServeHTTP(writer, request)
			if writer.status != http.StatusBadRequest {
			...
			if got, want := writer.String(), "invalid_last_event_id\n"; got != want {
