...
func TestChatModelAliasBuildsDefaultUtilityAndProfileRouting(t *testing.T) {
	...
	a, cleanup, err := buildAgentWithProfileRuntime(ctx, cfg, memory.Workspace{Dir: t.TempDir()}, nil, session.New(st), config.GroupMain, "research", runtime, nil, apiBrokerWiring{})
	if err != nil {
		t.Fatal(err)
	}
	defer cleanup()
...
func TestServeModelAliasSharesRuntimeAcrossAgents(t *testing.T) {
	...
	agents, _, _, _, _, cleanup, err := buildGatewayAgentsWithRuntime(ctx, cfg, memory.Workspace{Dir: t.TempDir()}, nil, session.New(st), runtime, nil, apiBrokerWiring{})
	if err != nil {
		t.Fatal(err)
	}
	defer cleanup()
...
func TestChatModelAliasChildProfileUsesResolvedTokenDefault(t *testing.T) {
	...
	a, cleanup, err := buildAgentWithProfileRuntime(ctx, cfg, memory.Workspace{Dir: t.TempDir()}, nil, session.New(st), config.GroupMain, "", runtime, nil, apiBrokerWiring{})
	if err != nil {
		t.Fatal(err)
	}
	defer cleanup()
